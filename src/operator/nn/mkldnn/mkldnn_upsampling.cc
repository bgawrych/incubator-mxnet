...
bool SupportMKLDNNUpSampling(const UpSamplingParam &param,
  ...
  const auto input_dtype = input[0].dtype();
  const int out_dtype = output.dtype();

  if(!common::ContainsOnlyStorage(input, mxnet::kDefaultStorage)
     || !(input_dtype == mshadow::kFloat32 || input_dtype == mshadow::kBfloat16)
  ...
  if (param.sample_type == up_enum::kBilinear) {
    ...
    auto kernel_dtype = input[1].dtype();
...
void MKLDNNLUpSamplingForward(const nnvm::NodeAttrs& attrs,
  ...
  auto fwd = GetUpSamplingFwd(param, ctx.is_train, in_data[0], out_data[0]);

  auto in_mem = in_data[0].GetMKLDNNData();
  auto out_mem = out_data[0].GetMKLDNNData(fwd.pd.dst_desc());
